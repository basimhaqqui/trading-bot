...
class ShadowResearchTests(unittest.TestCase):
    ...
    def test_fast_prediction_candidates_cluster_to_one_event(self):
        ...
        forecast = self.audit.forecasts()[0]
        self.assertEqual(
            forecast.specialist_id, "prediction-market-fast-settlement-baseline-v10"
        )
    ...
    def test_fast_prediction_uses_recorded_close_not_expected_expiration(self):
        ...
        expected_expiration = self.now + timedelta(days=2)
        for event in (
            self.event(
                "fast-close-anchor-rule",
                MarketEventType.CONTRACT_RULE,
                market,
                {
                    ...
                    "latest_expiration_time": expected_expiration.isoformat(),
                },
                event_time=self.now - timedelta(minutes=1),
            ),
            self.event(
                "fast-close-anchor-book",
                MarketEventType.BOOK_SNAPSHOT,
                market,
                {"yes_bids": [["0.45", "10"]], "no_bids": [["0.53", "10"]]},
                event_time=self.now,
        ...
        self.assertEqual(generated.appended, 1)
        forecast = self.audit.forecasts()[0]
        self.assertEqual(forecast.valid_until, close_time)
        self.assertEqual(forecast.values["registered_close_time"], close_time.isoformat())
        self.assertEqual(
            forecast.values["settlement_deadline"],
            (close_time + timedelta(hours=1, minutes=15)).isoformat(),
        )

    def test_fast_prediction_rejects_close_outside_preregistered_horizon(self):
        market = Instrument(
            "kalshi:prediction:FAST-LATE-CLOSE",
            "kalshi",
            "FAST-LATE-CLOSE",
            AssetClass.PREDICTION,
            "USD",
        ...
        self.store.register_instrument(market)
        close_time = self.now + timedelta(hours=2, seconds=1)
        for event in (
            self.event(
                "fast-late-close-rule",
                MarketEventType.CONTRACT_RULE,
                market,
                {
    ...
    def test_fast_prediction_v9_accepts_recorded_early_close_within_deadline(self):
        market = Instrument(
        ...
        self.store.append_event(
            self.event(
                ...
                market,
                {
                    ...
                    "event_ticker": "FAST-EARLY-EVENT",
                    "raw_market": {
                        "close_time": (expiration - timedelta(minutes=2)).isoformat()
                    },
                },
    ...
    def test_fast_prediction_v9_rejects_early_label_without_early_close_policy(self):
        market = Instrument(
            "kalshi:prediction:FAST-POLICY-INCONSISTENT",
            "kalshi",
            "FAST-POLICY-INCONSISTENT",
            AssetClass.PREDICTION,
            "USD",
        )
        self.store.register_instrument(market)
        ...
        self.assertEqual(self.runner.generate_forecasts(as_of=self.now).appended, 1)
        forecast = self.audit.forecasts()[0]
        self.assertEqual(forecast.specialist_id, "prediction-market-fast-settlement-baseline-v10")
        self.store.append_event(
            self.event(
    ...
    def test_fast_prediction_v9_requires_recorded_early_close_for_early_label(self):
        market = Instrument(
            "kalshi:prediction:FAST-V8-EARLY",
            "kalshi",
            "FAST-V8-EARLY",
            AssetClass.PREDICTION,
            "USD",
        ...
        self.store.append_event(
            self.event(
                "fast-v8-uncorroborated-settlement",
                MarketEventType.SETTLEMENT,
                market,
                {"result": "yes", "event_ticker": "FAST-V8-EVENT"},
                event_time=early_settlement,
            )
        )
        self.assertEqual(self.runner.score_available(as_of=early_settlement).appended, 0)

        self.store.append_event(
            self.event(
                "fast-v8-corroborated-settlement",
                MarketEventType.SETTLEMENT,
                market,
                {
                    "result": "yes",
        ...
        scored = self.runner.score_available(as_of=early_settlement + timedelta(seconds=1))
        self.assertEqual(scored.appended, 1)
        self.assertEqual(scored.due_unmatched, 0)

    def test_fast_prediction_v9_rejects_equal_close_and_finalization_times(self):
        market = Instrument(
            "kalshi:prediction:FAST-V9-EQUAL",
            "kalshi",
            "FAST-V9-EQUAL",
            AssetClass.PREDICTION,
            "USD",
        ...
        self.store.append_event(
            self.event(
                "fast-v9-equal-close-settlement",
                MarketEventType.SETTLEMENT,
                market,
                {
                    "result": "yes",
                    "event_ticker": "FAST-V9-EQUAL-EVENT",
                    "raw_market": {"close_time": early_finalization.isoformat()},
                },
                event_time=early_finalization,
            )
        )

        scored = self.runner.score_available(as_of=early_finalization)
