...
PREDICTION_FAST_SETTLEMENT_V9_HYPOTHESIS = Hypothesis(
    hypothesis_id="prediction-market-fast-settlement-baseline-v9",
    family="prediction-market-fast-settlement",
    market=AssetClass.PREDICTION,
    mechanism=(
        "The executable probability of a short-dated, active binary market is a fixed "
        "baseline for a fast finalization. This lane is intentionally unadjusted. "
        "Kalshi documents that trading stops when close_time passes and that close_time "
        "may move earlier only when can_close_early is true. An early finalization "
        "therefore counts only when the forecast recorded that boolean as true and the "
        "later public finalization response records a close_time strictly after forecast "
        "generation and strictly before both finalization and expected expiration. Each "
        "forecast otherwise accepts only its own event's public finalization from "
        "expected expiration through the pre-recorded expected-expiration plus "
        "settlement-timer plus one-hour deadline. Late, absent, and insufficiently "
        "corroborated labels remain unscored and cannot be reclassified into evidence."
    ),
    ...
    proposed_at=PREDICTION_FAST_V9_PROPOSED_AT,
)


PREDICTION_FAST_SETTLEMENT_V10_HYPOTHESIS = Hypothesis(
    hypothesis_id="prediction-market-fast-settlement-baseline-v10",
    family="prediction-market-fast-settlement",
    market=AssetClass.PREDICTION,
    mechanism=(
        "The executable probability of a short-dated, active binary market is a fixed "
        "baseline for a fast finalization. This lane is intentionally unadjusted. "
        "Kalshi documents close_time as when trading stops, while expected expiration "
        "is only when the outcome is expected to be known and can precede a much later "
        "close. Therefore a forecast is eligible only when its observed close_time is "
        "20 minutes to two hours away. Each forecast clusters all child contracts by "
        "the recorded event ticker and accepts only that event's public finalization "
        "through the pre-recorded close_time plus settlement timer plus one-hour "
        "deadline. An earlier finalization counts only when can_close_early was true "
        "at forecast time and the final response records a close_time strictly after "
        "forecast generation and strictly before both finalization and the registered "
        "close. Late, absent, and insufficiently corroborated labels remain unscored."
    ),
    ...
    proposed_at=PREDICTION_FAST_V10_PROPOSED_AT,
)


PREDICTION_FAST_SETTLEMENT_V11_HYPOTHESIS = Hypothesis(
    hypothesis_id="prediction-market-fast-settlement-baseline-v11",
    family="prediction-market-fast-settlement",
    market=AssetClass.PREDICTION,
    mechanism=(
        "The executable probability of a short-dated, active binary market is a fixed "
        "baseline for a fast finalization. This lane is intentionally unadjusted. "
        "Kalshi documents that close_time may change and that an active market may be "
        "deactivated or reopened. Therefore a forecast is eligible only when its "
        "contract-rule snapshot was available no more than fifteen minutes before the "
        "executable book, and the observed close_time is 20 minutes to two hours away. "
        "Each forecast clusters all child contracts by the recorded event ticker and "
        "accepts only that event's public finalization through the pre-recorded "
        "close_time plus settlement timer plus one-hour deadline. An earlier "
        "finalization counts only when can_close_early was true at forecast time and "
        "the final response records a close_time strictly after forecast generation "
        "and strictly before both finalization and the registered close. Late, absent, "
        "and insufficiently corroborated labels remain unscored."
    ),
    target="binary finalized settlement probability for a pre-registered fast-close window",
    horizon="20 minutes to two hours until recorded close_time; finalization no later than one hour plus the recorded settlement timer afterward",
    information_set=(
        "current executable yes and no bids",
        "a contract-rule snapshot available no more than fifteen minutes before the executable book",
        "Kalshi event ticker, recorded close_time, expected expiration time, and latest expiration time",
        "active market status and documented boolean can_close_early policy",
        "a public finalization response's close_time strictly between forecast generation and finalization for any early label",
        "a settlement timer no longer than fifteen minutes",
    ...
    invalidation_conditions=(
        ...
        "results disappear when one event ticker or resolution period is removed",
        "the fixed fast-close window leaves insufficient independent outcomes",
        "late, missing, policy-inconsistent, uncorroborated, or stale-rule label rates indicate that the lane is not operationally representative",
        "cursor order or page coverage explains the apparent performance",
        "the baseline does not survive fees, slippage, latency, and doubled-cost stress",
    ),
    proposed_at=PREDICTION_FAST_V11_PROPOSED_AT,
)
...
PREDICTION_FAST_SETTLEMENT_V13_HYPOTHESIS = Hypothesis(
    hypothesis_id="prediction-market-fast-settlement-baseline-v13",
    family="prediction-market-fast-settlement",
    market=AssetClass.PREDICTION,
    mechanism=(
        "The executable probability of a short-dated, active binary market is a fixed "
        "baseline for a fast finalization. This successor retains the v12 lifecycle, "
        "close-time, timing, provisional-market, and independence rules, and also "
        "requires the final public market response to prove that its close_time did not "
        "move later than the close_time recorded at forecast generation. Kalshi documents "
        "that a closed market can reopen when close_time moves into the future, so such a "
        "reschedule cannot become an outcome for the original fast-close cohort. This lane "
        "is intentionally unadjusted and does not reuse earlier fast-lane cohorts."
    ),
    target="binary finalized settlement probability for a pre-registered fast-close window",
    horizon="20 minutes to two hours until recorded close_time; finalization no later than one hour plus the recorded settlement timer afterward",
    information_set=(
    ...
    proposed_at=PREDICTION_FAST_V13_PROPOSED_AT,
)


PREDICTION_FAST_SETTLEMENT_V14_HYPOTHESIS = Hypothesis(
    hypothesis_id="prediction-market-fast-settlement-baseline-v14",
    family="prediction-market-fast-settlement",
    market=AssetClass.PREDICTION,
    mechanism=(
        "The executable probability of a short-dated, active binary market is a fixed "
        "baseline for a fast finalization. This successor retains v13's lifecycle, "
        "close-time, timing, provisional-market, event-identity, and independence "
        "rules. It additionally requires an early final close_time to be consistent "
        "with the can_close_early value recorded at forecast generation. Kalshi "
        "documents that a close_time may move earlier only when can_close_early is "
        "true; an earlier final close under any other recorded policy cannot become "
        "an outcome for this cohort. This lane is intentionally unadjusted and does "
        "not reuse earlier fast-lane cohorts."
    ),
    target="binary finalized settlement probability for a pre-registered fast-close window",
    horizon="20 minutes to two hours until recorded close_time; finalization no later than one hour plus the recorded settlement timer afterward",
    information_set=(
    ...
    proposed_at=PREDICTION_FAST_V14_PROPOSED_AT,
)


PREDICTION_FAST_SETTLEMENT_V15_HYPOTHESIS = Hypothesis(
    hypothesis_id="prediction-market-fast-settlement-baseline-v15",
    family="prediction-market-fast-settlement",
    market=AssetClass.PREDICTION,
    mechanism=(
        "The executable probability of a short-dated, active binary market is a fixed "
        "baseline for a fast finalization. This successor retains v14's lifecycle, "
        "close-time, timing, event-identity, and independence rules, but requires the "
        "public market response to explicitly report is_provisional=false at forecast "
        "generation. Kalshi documents that a true provisional market may disappear from "
        "the public API at settlement; an absent flag is therefore treated as unknown, "
        "not as a safe negative. This lane is intentionally unadjusted and does not "
        "reuse earlier fast-lane cohorts."
    ),
    target="binary finalized settlement probability for a pre-registered fast-close window",
    horizon="20 minutes to two hours until recorded close_time; finalization no later than one hour plus the recorded settlement timer afterward",
    ...
    proposed_at=PREDICTION_FAST_V15_PROPOSED_AT,
)
...
BASELINE_HYPOTHESIS_SPECIALIST_IDS = {
    ...
    PREDICTION_FAST_SETTLEMENT_V9_HYPOTHESIS.hypothesis_id: (
        "prediction-market-fast-settlement-baseline-v9",
    ),
    PREDICTION_FAST_SETTLEMENT_V10_HYPOTHESIS.hypothesis_id: (
    ...
    PREDICTION_FAST_SETTLEMENT_V13_HYPOTHESIS.hypothesis_id: (
        "prediction-market-fast-settlement-baseline-v13",
    ),
    PREDICTION_FAST_SETTLEMENT_V14_HYPOTHESIS.hypothesis_id: (
