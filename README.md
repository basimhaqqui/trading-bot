...
- Fast-settlement prediction v16 requires an `active` binary market that explicitly reports `is_provisional=false`, with an executable book and lifecycle-rule snapshot no more than fifteen minutes apart, recorded `close_time` 20 minutes to two hours ahead, a settlement timer of at most fifteen minutes, and a maximum ten-cent spread. It anchors the label window to the recorded `close_time`, not expected or latest expiration: every final response must provide a close time strictly after forecast generation and strictly before finalization, and it must be no later than the registered close. An earlier final close is eligible only when the forecast recorded `can_close_early=true`; this excludes labels after a documented close-date extension or reopen and labels from an undocumented early close. Each documented `event_ticker` supplies at most one tightest-spread candidate and one scored outcome, so related strikes never inflate evidence. Delayed, missing, policy-inconsistent, uncorroborated, provisional-market, and rescheduled-close labels remain unscored and are never recategorized as evidence. The rules follow Kalshi's [market lifecycle documentation](https://docs.kalshi.com/getting_started/market_lifecycle), [settlement documentation](https://docs.kalshi.com/getting_started/market_settlement), [market-list reference](https://docs.kalshi.com/api-reference/market/get-markets), and [glossary](https://docs.kalshi.com/getting_started/terms).
- The rapid Kalshi collector restarts a bounded two-hour close-time query every cycle, then retains only API-reported `active` markets. This follows Kalshi's documented constraint that close-time filters cannot be combined with `status=open`; it prevents a cursor through the broader listing from skipping contracts that newly enter the preregistered cohort.
...
configured dedicated read-only endpoint, never the shared public RPC. The
...
The separately preregistered fast-settling v16 baseline is not an adjustment and cannot reuse v15 or earlier fast-lane cohorts. It accepts only active Kalshi contracts whose documented `market_type` is exactly `binary`, whose lifecycle-rule snapshot is no more than fifteen minutes before its executable book, and which explicitly report `is_provisional=false`; an absent, scalar, `true`, or malformed value is unknown and excluded. It also requires a documented boolean `can_close_early` policy, a settlement timer of at most fifteen minutes, a spread no wider than ten cents, and a recorded `close_time` more than 20 minutes and no more than two hours ahead. It records `expected_expiration_time` and `latest_expiration_time` for provenance but does not use either as the scored boundary: Kalshi documents `close_time` as the time trading stops, while expected expiration is only when the outcome is expected to be known and latest expiration is the latest possible expiry. A same-event finalization is scoreable only from the registered close through the immutable close-plus-settlement-timer-plus-one-hour deadline, and every final response must record a `close_time` strictly after forecast generation and strictly before finalization. That final close time must be no later than the registered close, and any earlier final close is scoreable only when the forecast recorded `can_close_early=true`; therefore a documented close-date extension, reopen, or undocumented early close cannot produce a v16 label. Missing, delayed, policy-inconsistent, nonbinary, provisional-market, and rescheduled-close timestamps remain unscored. Every rapid cycle starts a new bounded two-hour `min_close_ts`/`max_close_ts` market query with `status` omitted, as Kalshi documents that close-time filters are incompatible with `status=open`; the collector then retains only REST-reported `active` non-MVE responses. One lowest-spread candidate and one scored outcome may represent each documented `event_ticker`, regardless of correlated strikes. See Kalshi's [market lifecycle documentation](https://docs.kalshi.com/getting_started/market_lifecycle), [Kalshi glossary](https://docs.kalshi.com/getting_started/terms), [API changelog](https://docs.kalshi.com/changelog), and [public market-list reference](https://docs.kalshi.com/api-reference/market/get-markets).
