...
- Fast-settlement prediction v7 records Kalshi's latest possible expiry without treating it as a short-horizon promise; it accepts only same-event finalizations from expected expiration through a fixed settlement-timer and one-hour deadline, except for an earlier finalization when the forecast recorded Kalshi's `can_close_early=true` policy. Delayed, missing, and policy-inconsistent labels remain unscored and are never recategorized as evidence.
