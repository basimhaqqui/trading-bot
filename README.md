...
- Fast-settlement prediction v9 records Kalshi's latest possible expiry without treating it as a short-horizon promise; it accepts only same-event finalizations from expected expiration through a fixed settlement-timer and one-hour deadline, except for an earlier finalization when the forecast recorded `can_close_early=true` and the later public finalization response records a close time strictly after forecast generation and strictly before both finalization and expected expiration. Delayed, missing, policy-inconsistent, and uncorroborated early labels remain unscored and are never recategorized as evidence.
