...
The checked-in public plan collects active Kalshi binary markets closing within 48 hours, public settlements and trades, Coinbase product definitions and funding, executable BTC spot/perpetual books, and hourly candles for BTC, ETH, SOL, and DOGE. Multivariate Kalshi combinations are excluded because their scalar settlement does not match the pre-registered binary target. A bounded outcome job uses Kalshi's public exact-ticker filter to revisit unscored binary forecast markets once their expected occurrence or close time has passed; already scored markets, combinations, and contracts without a trustworthy lifecycle time are skipped. Candidate selection covers unforecasted underlying occurrences before revisiting any, chooses the tightest executable contract within each occurrence, and rejects spreads above the pre-declared ten-cent limit. The preregistered v3 cohort also requires a parseable occurrence more than one and no more than eight hours after the point-in-time book; the forecast validity boundary is that occurrence, and post-occurrence forecasts are never scored. This keeps every new prediction observation in one fixed `1h-8h` cohort. Calibration and evaluation use the same conservative occurrence cluster, so correlated props and strikes cannot satisfy the independent-outcome gate:
