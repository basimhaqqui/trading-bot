...
- Fast-settlement prediction v15 requires an `active` binary market that explicitly reports `is_provisional=false`, with an executable book and lifecycle-rule snapshot no more than fifteen minutes apart, recorded `close_time` 20 minutes to two hours ahead, a settlement timer of at most fifteen minutes, and a maximum ten-cent spread. It anchors the label window to the recorded `close_time`, not expected or latest expiration: every final response must provide a close time strictly after forecast generation and strictly before finalization, and it must be no later than the registered close. An earlier final close is eligible only when the forecast recorded `can_close_early=true`; this excludes labels after a documented close-date extension or reopen and labels from an undocumented early close. Delayed, missing, policy-inconsistent, uncorroborated, provisional-market, and rescheduled-close labels remain unscored and are never recategorized as evidence. The rules follow Kalshi's [market lifecycle documentation](https://docs.kalshi.com/getting_started/market_lifecycle), [settlement documentation](https://docs.kalshi.com/getting_started/market_settlement), and [API changelog](https://docs.kalshi.com/changelog).
